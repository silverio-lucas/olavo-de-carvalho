...
// ⚡️ DANGER ZONE ⚡️
// ================
// {% if jekyll.environment == 'production' or site.deepdive.offline.development %}
...
// {% if google_fonts %}
const GOOGLE_FONTS =
  "{{ site.google_fonts_url | default:'https://fonts.googleapis.com' }}/css?family={{ google_fonts | uri_escape }}&display=swap";
// {% endif %}
...
const PRE_CACHED_ASSETS = [
  ...
  /*{% for file in site.deepdive.offline.precache_assets %}*/ '{% include_cached smart-url url=file %}',
  /*{% endfor %}*/
];
...
async function getIconFontFiles() {
  const fontURLs = STATIC_FILES.filter(
    (x) => x.startsWith('{{ "/assets/icomoon/fonts/" | relative_url }}') && x.endsWith('.woff'),
  );
...
async function getKaTeXFontFiles() {
  const fontURLs = STATIC_FILES.filter(
    (x) => x.startsWith('{{ "/assets/bower_components/katex/dist/fonts/" | relative_url }}') && x.endsWith('.woff2'),
  );
