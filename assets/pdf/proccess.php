...
*/
$handle = opendir(".");
$list_ignore = array('photothumb.db', 'desktop.ini', 'makeindex.php', '.', '..', '.htaccess', 'proccess.php');
$cnt = 0;
$json = "";
while ($file = readdir($handle)) {
  if (!in_array($file, $list_ignore)) {
    echo '[' . $file . ']' . chr(13);
    if ($file != "") {
      $alt = str_replace('.pdf', '', $file);
      $alt = str_replace('.jpg', '', $alt);
      $alt = str_replace('.png', '', $alt);
      $alt = str_replace('.jpeg', '', $alt);

      $alt = str_replace('-olavo_de_carvalho', '', $alt);
      $alt = str_replace('_olavo_de_carvalho', '', $alt);
      $alt = str_replace('_olavo_de', '', $alt);
      $alt = str_replace('_olavo_1', '', $alt);

      $title = str_replace('_', ' ', $alt);
      $title = str_replace('-olavo_de_carvalho', '', $title);
      $title = str_replace('_olavo_de_carvalho', '', $title);
      $title = str_replace('_olavo_de', '', $title);
      $title = str_replace('_olavo_1', '', $title);
      $strcontent = '---
layout: post
published: true
image:
  path: /assets/img/' . substr($file, 0, strlen($file) - 4) . '.png
title: ' . $title . '
category: projects
tag: [biblioteca, filosofia]
description: A Escolástica - Aula 15 - por Olavo de Carvalho
projecturl: /assets/pdf/' . $file . '
---
';
      $myfile = fopen("./" .  $alt . ".md", "w");
